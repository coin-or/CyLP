...
// Perturbs problem
void 
IClpSimplexPrimal_Wolfe::perturb(int type)
{
	if (perturbation_>100)
...
// un perturb
bool
IClpSimplexPrimal_Wolfe::unPerturb()
{
	if (perturbation_!=101)
		return false;
...
int 
IClpSimplexPrimal_Wolfe::unflag()
{
	int i;
...
void 
IClpSimplexPrimal_Wolfe::alwaysOptimal(bool onOff)
{
	if (onOff)
		specialOptions_ |= 1;
	else
...
bool 
IClpSimplexPrimal_Wolfe::alwaysOptimal() const
{
	return (specialOptions_&1)!=0;
}
// Flatten outgoing variables i.e. - always to exact bound
void 
IClpSimplexPrimal_Wolfe::exactOutgoing(bool onOff)
{
	if (onOff)
		specialOptions_ |= 4;
	else
...
bool 
IClpSimplexPrimal_Wolfe::exactOutgoing() const
{
 ...
 */
int
IClpSimplexPrimal_Wolfe::pivotResult(int ifValuesPass)
{
...
// 	  std::cout << "Basis: " <<  listSize << std::endl;
// 	  for (int iii = 0 ; iii < listSize ; iii++)
// 	  {
// 		  //std::cout << (getStatus(iii) == basic) << std::endl;
// 		  if (getStatus(iii) == basic)
// 		  {
// 			  std::cout << iii << ", ";
// 			  
// 		  }
...
// 	  std::cout << std::endl;
	  ...
	  //int iIndex, iPivot, iRow;
	  
	  /*for (iIndex=0;iIndex<number;iIndex++) 
		  {
			  iRow = which[iIndex];
			  iPivot=pivotVariable_[iRow];
			  if (iRow == pivotRow_)
				  break;
			  //std::cout << iRow << ", " << iPivot << " : ";
		  }
		  */
		int iPivot = pivotVariable_[pivotRow_];	
	
	  //std::cout << std::endl;
	  std::cout << "iteration : " << numberIterations_ << "\n";
	  std::cout << "pivot row : " << iPivot << std::endl;
	 	std::cout << "pivot col : " << sequenceIn_ << std::endl;
	 std::cout << "comp of col : " << QP_ComplementarityList[sequenceIn_] << std::endl; 
	  for (int i = 0; i < listSize; i++)
// 	  	std::cout << QP_ComplementarityList[i] << ", ";
// 	  std::cout << "\n";
	  //std:: cout << getStatus(QP_ComplementarityList[sequenceIn_]) << "\n";
	  
	  //std::cout << "? : " << (getStatus(QP_ComplementarityList[sequenceIn_]) == basic) << std::endl;
	  if (pivotRow_ >= 0 && getStatus(QP_ComplementarityList[sequenceIn_]) == basic && 
					QP_ComplementarityList[sequenceIn_] == iPivot)
				std::cout << "IClpSimplex Primal: Primal In, Dual Out________________\n";
			  	
	  if (pivotRow_ >= 0 && getStatus(QP_ComplementarityList[sequenceIn_]) == basic && 
					QP_ComplementarityList[sequenceIn_] != iPivot)
		  {
			  std::cout << "IClpSimplex Primal: variable " << sequenceIn_ << " banned." << std::endl;
			  //QP_BanList[sequenceIn_] = 1;
			  //QP_ExistsBannedVariable = 1;
			  setFlagged(sequenceIn_);
		//std::cout << "+++++++++++++++++++++++++++++++++++++++++++++\n";
		//for (int ii = 0 ; ii < listSize ; ii++)
		//	std::cout << ii << " : " << QP_BanList[ii] << ", ";
		//std::cout << std::endl;
			returnCodeBack = -5;
			returnCode = -5;
			clearAll();
		...
		//TODO: We are heading to an actual pivot. Clear the banlist for the next iteration
...
// Create primal ray
void 
IClpSimplexPrimal_Wolfe::primalRay(CoinIndexedVector * rowArray)
{
	delete [] ray_;
 ...
 */
int 
IClpSimplexPrimal_Wolfe::nextSuperBasic(int superBasicType,CoinIndexedVector * columnArray)
{
...
void
IClpSimplexPrimal_Wolfe::clearAll()
{
	// Clean up any gub stuff
	matrix_->extendUpdated(this,rowArray_[1],1);
	int number=rowArray_[1]->getNumElements();
...
int
IClpSimplexPrimal_Wolfe::lexSolve()
{
	algorithm_ = +1;
	//specialOptions_ |= 4;
